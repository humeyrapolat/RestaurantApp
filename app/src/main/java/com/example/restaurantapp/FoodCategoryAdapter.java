...
public class FoodCategoryAdapter extends RecyclerView.Adapter<FoodCategoryAdapter.CategoryHolder> {
    ...
    public FoodCategoryAdapter(List<Category> data, Context context, OnCategoryClick onCategoryClick) {
        this.data = data;
        this.context = context;
        this.onCategoryClick = onCategoryClick;
    }

    public FoodCategoryAdapter(List<Category> data, Context context) {
        this.data = data;
        this.context = context;
    }
    ...
    public CategoryHolder onCreateViewHolder(@NonNull ViewGroup parent, int viewType) {
        ...
        View view = layoutInflater.inflate(R.layout.activity_categories, parent, false);
    ...
    public void onBindViewHolder(@NonNull CategoryHolder holder, int position) {
        ...
        if (position == selectedItem) {
            // Make card selected
            holder.cardView.setOutlineSpotShadowColor(context.getColor(R.color.darkred));
            holder.cardView.setOutlineAmbientShadowColor(context.getColor(R.color.darkred));
            holder.cardView.setStrokeWidth(3);
            holder.title.setTextColor(context.getColor(R.color.red));
        } else {
            // Make card inactive
            holder.cardView.setOutlineSpotShadowColor(context.getColor(R.color.black));
